...
int Message(HWND hWnd, LPCTSTR lpText, LPCTSTR lpCaption, UINT uType)
{
...
void check_key()
{
	...
	{
		Message(NULL,"The file SSHProxy.exe should be in the C:\\Program Files\\UnixSpace\\bin directory.", "Error",MB_OK | MB_ICONEXCLAMATION);
		exit(0);
	}
	read(fd,buf,sizeof buf);
	close(fd);
...
ASK:
	...
	Message(NULL,buf, "Warning",MB_OK | MB_ICONEXCLAMATION);
}
...
{ 
	...
	{
		Message(NULL, "Unable to initialise WinSock", "WinSock Error", MB_OK | MB_ICONEXCLAMATION); 
		return 1; 
	} 
	if (LOBYTE(wsadata.wVersion) != 1 || HIBYTE(wsadata.wVersion) != 1)
	{
		Message(NULL, "WinSock version is incompatible with 1.1", "WinSock Error", MB_OK | MB_ICONEXCLAMATION); 
		WSACleanup(); 
		return 1; 
	} 
...
//	defuse_showwindow(); 
	...
	{ 
		...
		Message(NULL, "Windows refuses to report a version",  str, MB_OK | MB_ICONEXCLAMATION); 
		sfree(str); 
		return 1; 
	} 
...
*/
	...
	{ 
...

		{
			...
			/// WinSock DLL.                                  
SOCK_ERR:
				Message(hwnd, mess,"WinSock Error", MB_OK | MB_ICONEXCLAMATION);
				DestroyWindow(hwnd); 
				exit(0);
			}
...
//		ReuseAddress();
...
#ifdef _DEBUG
			{
				...
				CreateProcess("C:/CX5/bin/UnixSpace.exe", NULL, NULL, NULL, 0, 0, NULL, NULL, &StartInfo, &PInfo);
			}
#endif
...
//		strcpy(msg,"www.unixspace.com.ua");
		/** 
	...
	{ 
		...
		{ 
			...
			for (i = 0; i < argc; i++)
			{
				...
				else if (!strncmp(p, "-proxy:",6))
				{
					sscanf(p,"-proxy:type=%d;port=%d;host=%s",&cfg.proxy_type,&cfg.proxy_port,cfg.proxy_host);
				}
				else if (!strcmp(p, "-cleanup"))
				{
					...
					if (Message(NULL, s1, s2, MB_YESNO | MB_ICONWARNING) == IDYES)
					{
						cleanup_all(); 
					} 
					sfree(s1); 
	...
	{ 
		...
		if (back == NULL)
		{
			...
			Message(NULL, "Unsupported protocol number found", str, MB_OK | MB_ICONEXCLAMATION); 
			sfree(str); 
			WSACleanup(); 
			return 1; 
		} 
	...
	{
		char *str = dupprintf("%s Internal Error", appname); 
		Message(NULL, "Invalid Port Number", str, MB_OK | MB_ICONEXCLAMATION); 
		sfree(str); 
		WSACleanup(); 
		return 1; 
	} 
		...
		if (error)
		{
			...
			sprintf(msg, "Unable to open connection to\n" 
			    "%.800s\n" "%s", cfg.host, error); 
			Message(NULL, msg, str, MB_ICONERROR | MB_OK); 
			sfree(str); 
			return 0; 
		} 
...
{ 
	...
	Message(hwnd, stuff, morestuff, MB_ICONERROR | MB_OK); 
	sfree(stuff); 
...
//			set_icon(NULL, morestuff); 
//			set_title(NULL, morestuff); 
			Message(hwnd, "Connection closed by remote host", appname, MB_OK | MB_ICONINFORMATION); 
		} 
	} 
} 
