...
#define ADD_BIGNUM(bignum) numlen = (bignum_bitcount(bignum)+8)/8; PUT_32BIT(lenbuf, numlen); MD5Update(&md5c, lenbuf, 4); for (i = numlen; i-- ;) { unsigned char c = bignum_byte(bignum, i); MD5Update(&md5c, &c, 1); }
	ADD_BIGNUM(dss->p); 
	ADD_BIGNUM(dss->q); 
	ADD_BIGNUM(dss->g); 
	ADD_BIGNUM(dss->y); 
...
{ 
	...
	bloblen = 0; 
#define ENC(x) PUT_32BIT(blob+bloblen, ssh2_bignum_length((x))-4); bloblen += 4; for (i = ssh2_bignum_length((x))-4; i-- ;) blob[bloblen++]=bignum_byte((x),i);
	ENC(dss->p); 
	ENC(dss->q); 
	ENC(dss->g); 
	ENC(dss->y); 
