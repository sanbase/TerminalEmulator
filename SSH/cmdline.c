...
#define SAVEABLE(pri) do { if (need_save) { cmdline_save_param(p, value, pri); return ret; } } while (0)
...
#define UNAVAILABLE_IN(flag) do { if (cmdline_check_unavailable(flag, p)) return ret; } while (0) 
...
#define RETURN(x) do { if ((x) == 2 && !value) return -2; ret = x; if (need_save < 0) return x; } while (0)
...
{ 
	...
	for (pri = 0; pri < NPRIORITIES; pri++) 
		for (i = 0; i < saves[pri].nsaved; i++) 
			cmdline_process_param(saves[pri].params[i].p,saves[pri].params[i].value, 0, cfg);
} 
