...
#define crState(t) struct t *s; if (!ssh->t) ssh->t = snew(struct t); s = ssh->t;
#define crFinish(z)	} *crLine = 0; return (z); } 
#define crFinishV	} *crLine = 0; return; } 
#define crReturn(z)     do {*crLine =__LINE__; return (z); case __LINE__:;} while (0)
#define crReturnV       do {*crLine=__LINE__; return; case __LINE__:;} while (0)
...
static int nullmac_verify(void *handle, unsigned char *blk, int len, unsigned long seq)
{ 
	return 1; 
} 
...
#define bombout(msg) do { char *text = dupprintf msg; ssh_do_close(ssh); logevent(text); connection_fatal(ssh->frontend, "%s", text); sfree(text); } while (0)
...
next_packet: 
	...
	if (ssh->cipher && detect_attack(ssh->crcda_ctx, ssh->pktin.data, st->biglen, NULL))
	{
...
{ 
	...
	s->i = 0; 
	while (1) { 
		static const int transS[] =
		{
			1, 2, 2, 1
		};
		static const int transH[] =
		{
			0, 0, 3, 0
		};
		static const int transminus[] =
		{
			0, 0, 0, -1
		};
		if (c == 'S') 
...
//			askcipher(ssh->frontend, cipher_string, 0); 
	...
	{ 
		...
		{ 
			...
			logevent(userlog); 
			if (flags & FLAG_INTERACTIVE && (!((flags & FLAG_STDERR) && (flags & FLAG_VERBOSE))))
			{
				strcat(userlog, "\r\n"); 
	...
	while (ssh->pktin.type == SSH1_SMSG_FAILURE)
	{
		...
		if (ssh->cfg.try_tis_auth && (s->supported_auths_mask & (1 << SSH1_AUTH_CCARD)) && !s->ccard_auth_refused)
		{
...
{ 
	...
	if (ssh->cfg.agentfwd && agent_exists())
	{
		...
		while (!ispkt); 
		if (ssh->pktin.type != SSH1_SMSG_SUCCESS && ssh->pktin.type != SSH1_SMSG_FAILURE)
		{
	...
	while (1)
	{
		...
		if (ispkt)
		{
			if (ssh->pktin.type == SSH1_SMSG_STDOUT_DATA || ssh->pktin.type == SSH1_SMSG_STDERR_DATA)
			{
			...
			else if (ssh->pktin.type == SSH1_SMSG_X11_OPEN)
			{
				...
				else
				{
					...
					if (x11_init(&c->u.x11.s, ssh->cfg.x11_display, c, ssh->x11auth, NULL, -1, &ssh->cfg) != NULL)
					{
			...
			else if (ssh->pktin.type == SSH1_MSG_PORT_OPEN)
			{
				...
				for (h = host, p = (char *)(ssh->pktin.body+8); hostsize != 0; hostsize--)
				{
			...
			else if (ssh->pktin.type == SSH1_MSG_CHANNEL_CLOSE || ssh->pktin.type == SSH1_MSG_CHANNEL_CLOSE_CONFIRMATION)
			{
...
{ 
	...
	do
	{
		...
		else if ((flags & FLAG_INTERACTIVE) && !*ssh->cfg.username)
		{
			if (ssh_get_line && !ssh_getline_pw_only)
			{
				if (!ssh_get_line("\n\rLogin: ", s->username, sizeof(s->username), FALSE))
				{
					/** 
					     * get_line failed to get a username.
					     * Terminate.
		...
		while (1)
		{
			...
			if (s->kbd_inter_running && ssh->pktin.type == SSH2_MSG_USERAUTH_INFO_REQUEST)
			{
				/** 
			...
			if (ssh->pktin.type == SSH2_MSG_USERAUTH_FAILURE)
			{
				...
				if (!ssh2_pkt_getbool(ssh))
				{
					...
					else if (s->type == AUTH_TYPE_PUBLICKEY_OFFER_LOUD || s->type == AUTH_TYPE_PUBLICKEY_OFFER_QUIET)
					{
			...
			if (!s->method && s->can_pubkey && 
				...
				if (!agent_query(s->request, 5, &r, &s->responselen, ssh_agent_callback, ssh))
				{
					do
					{
						crReturnV; 
				...
				s->response = (unsigned char *) r; 
				if (s->response && s->responselen >= 5 && s->response[4] == SSH2_AGENT_IDENTITIES_ANSWER)
				{
					s->p = s->response + 5; 
					s->nkeys = GET_32BIT(s->p); 
					s->p += 4; 
					...
					for (s->keyi = 0; s->keyi < s->nkeys; s->keyi++)
					{
						...
						*s->q++ = SSH2_AGENTC_SIGN_REQUEST; 
						...
						PUT_32BIT(s->q, 0); 
						if (!agent_query(s->agentreq, s->len + 4, &vret, &s->retlen, ssh_agent_callback, ssh))
						{
							do
							{
								crReturnV; 
			...
			if (!s->method && s->can_pubkey && s->publickey_blob && !s->tried_pubkey_config)
			{
			...
			if (!s->method && s->can_passwd)
			{
				...
				sprintf(s->pwprompt, "Password: ", s->username, 
				ssh->savedhost); 
				s->need_pw = TRUE; 
			} 
	...
	while (1)
	{
		...
		if (ispkt)
		{
			if (ssh->pktin.type == SSH2_MSG_CHANNEL_DATA || ssh->pktin.type == SSH2_MSG_CHANNEL_EXTENDED_DATA)
			{
				char *data; 
				int length; 
				unsigned i = ssh2_pkt_getuint32(ssh); 
...
{ 
	...
	ssh->logctx = NULL; 
	{ 
		static const struct Packet empty =
		{
			0, 0, NULL, NULL, 0
		};
