...
#define ECHOING (ldisc->cfg->localecho == FORCE_ON || (ldisc->cfg->localecho == AUTO && (ldisc->back->ldisc(ldisc->backhandle, LD_ECHO) ))) 
#define EDITING (ldisc->cfg->localedit == FORCE_ON || (ldisc->cfg->localedit == AUTO && (ldisc->back->ldisc(ldisc->backhandle, LD_EDIT) ))) 
...
{ 
//f ((c >= 32 && c <= 126) || (c >= 160 && !in_utf(ldisc->term))) 
	if ((c >= 32 && c <= 255))
		return 1; 
	else if (c < 128) 
