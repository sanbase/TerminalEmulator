...
static int plug_proxy_closing (Plug p, const char *error_msg, int error_code, int calling_back)
{ 
 ...
 */ 
static int proxy_for_destination (SockAddr addr, char *hostname, int port, const Config *cfg)
{ 
	int s = 0, e = 0; 
	char hostip[64]; 
...
SockAddr name_lookup(char *host, int port, char **canonicalname, const Config *cfg)
{ 
	if (cfg->proxy_type != PROXY_NONE && do_proxy_dns(cfg) && proxy_for_destination(NULL, host, port, cfg))
	{
...
Socket new_connection(SockAddr addr, char *hostname, int port, int privport, int oobinline, int nodelay, Plug plug, const Config *cfg)
{ 
...
#define ENSURE(n) do { \
	...
	ret = sresize(ret, retsize, char);}} while (0) 
