...
int makekey(unsigned char *data, struct RSAKey *result, unsigned char **keystr, int order)
{ 
...
{ 
	...
	bloblen = 0; 
#define ENC(x) PUT_32BIT(blob+bloblen, ssh2_bignum_length((x))-4); bloblen += 4; \
...
#define ADD_BIGNUM(bignum) \
	numlen = (bignum_bitcount(bignum)+8)/8; PUT_32BIT(lenbuf, numlen); MD5Update(&md5c, lenbuf, 4); \
	for (i = numlen; i-- ;) { \
