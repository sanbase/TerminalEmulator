	...
	: CDialog(Registration::IDD, pParent)
{
	...
	if(stat("C:/Program Files/UnixSpace/bin/cx5.exe",&st)<0)
	{
...
void Registration::OnDone() 
{
	...
	if((fd=creat("C:/Program Files/UnixSpace/etc/CX5.key",_S_IREAD|_S_IWRITE))>0)
	{
		char str[64];
		sprintf(str,"%d",m_Key);
	...
	else AfxMessageBox("Can't create file C:\\Program Files\\UnixSpace\\etc\\CX5.key");
	EndDialog(true);
}
